...
public class Dates {
    ...
    private final static int SHORT_ISO_8601_TIME_LENGTH =          "1994-11-05T08:15:30Z".length();
    private final static int LOCAL_DATE_TIME_SHORT =               "2016-05-05T00:00".length();
    private final static int LOCAL_DATE_TIME_LONG =                "2016-05-05T00:00:01.000000001".length();
    private final static int INDEX_OF_T_IN_LOCAL_DATE_TIME =       "2016-05-05T00:00:01.000000001".indexOf('T');
    private final static int INDEX_OF_COLON_IN_LOCAL_DATE_TIME =   "2016-05-05T00:00:01.000000001".indexOf(':');
    private final static int INDEX_OF_DASH_IN_LOCAL_DATE_TIME =    "2016-05-05T00:00:01.000000001".indexOf('-');
    private final static int LONG_ISO_8601_TIME_LENGTH =           "1994-11-05T08:15:30-05:00".length();
    private final static int LONG_ISO_8601_JACKSON_TIME_LENGTH =   "1994-11-05T08:11:22.123-0500".length();
    ...
    public static boolean isLocalDateTime(char[] chars) {
        ...
        if (length >= LOCAL_DATE_TIME_SHORT &&
                length <= LOCAL_DATE_TIME_LONG) {
            return chars[INDEX_OF_T_IN_LOCAL_DATE_TIME] == 'T' &&
                    chars[INDEX_OF_COLON_IN_LOCAL_DATE_TIME] == ':' &&
                    chars[INDEX_OF_DASH_IN_LOCAL_DATE_TIME] == '-';
        }
        return false;
    }

    public static LocalDateTime parseLocalDateTime(char[] chars) {

        final char[][] parts = CharScanner.splitByCharsNoneEmpty(chars, '-', ':', 'T', '.', 'Z');
        int year = 0;
        int month = 0;
        int day = 0;
        ...
        if (parts.length >= 7) {
            nanoOfSecond = CharScanner.parseInt(parts[6]);
        }

        return LocalDateTime.of(year, month, day, hour, minutes, seconds, nanoOfSecond);
    }
    ...
    public static boolean isISO8601QuickCheck(char[] charArray) {
        ...
        if (length == JSON_TIME_LENGTH
                || length == LONG_ISO_8601_TIME_LENGTH
                || length == SHORT_ISO_8601_TIME_LENGTH
                || length == LOCAL_DATE_TIME_SHORT
                || (length >= 17 && (charArray[16] == ':'))
                ) {
            return true;
        }
    ...
    public static boolean isShortDate(char[] chars) {
        if (chars.length > 13) {
            return chars[13] == ':';
        } else {
            return false;
