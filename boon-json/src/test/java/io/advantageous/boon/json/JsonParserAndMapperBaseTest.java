...
import static io.advantageous.boon.core.IO.putl;
...
public class JsonParserAndMapperBaseTest {
    ...
    public void testFilesFromReader() throws Exception {
        ...
        for ( String file : list ) {
            ...
            try {
                Object object =  jsonParserAndMapper.parse ( Files.newInputStream ( Paths.get(file) ) );
    ...
    public void parseArray () {
        ...
        boolean ok = Arrays.equals (compareArray, array) || die( Str.sputs(array));


        final long[] larray = jsonParserAndMapper.parseLongArray ( testString );

        ok = Arrays.equals (compareLongArray, larray) || die( Str.sputs(larray));


        final byte[] barray = jsonParserAndMapper.parseByteArray ( testString );
        ok |= Arrays.equals (compareByteArray, barray) || die( Str.sputs(barray));

        final short[] sarray = jsonParserAndMapper.parseShortArray ( testString );
        ok |= Arrays.equals (compareShortArray, sarray) || die( Str.sputs(sarray));

        final float[] farray = jsonParserAndMapper.parseFloatArray ( testString );
        ok |= Arrays.equals (compareFloatArray, farray) || die( Str.sputs(farray));

        final double[] darray = jsonParserAndMapper.parseDoubleArray ( testString );
        ok |= Arrays.equals (compareDoubleArray, darray) || die( Str.sputs(darray));
